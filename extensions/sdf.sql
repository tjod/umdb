Create Temporary View atom_block As Select atom.molecule_id, count(atom.atom_number) natoms,
 group_concat(printf("%10.4f%10.4f%10.4f %-2s%3d%3.0f%3d%3d%3d",coord.x,coord.y,coord.z,symbol,0,charge,0,0,0,0),char(10)) records
 --From atom Left Join coord On (atom.molecule_id=coord.molecule_id And atom.atom_number=coord.atom_number) 
 From atom Left Join coord Using (molecule_id, atom_number)
 Group By atom.molecule_id Order By atom.atom_number Asc ;
...
Create Temporary View atom_list_block As Select molecule_id, group_concat(printf("M  CHG%3d %3d %3d",1,atom_number,charge), char(10)) records From atom
 Where charge!=0 Group By molecule_id;

-- each section as a column
Create Temporary View molblock As
Select molecule_id, name, printf("  %-8s%10.10s", 'UMDBv1',created) line2, "" line3,
...
Select molecule_id, coalesce(name,'')||char(10)||
...
-- Select molblock from sdf where molecule_id in (Select molecule_id from molecule where name like '%chloride%') Order By molecule_id Asc;
